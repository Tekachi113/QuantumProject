...
if not comp_data or not hw_data:
    st.error("Không tìm thấy file kết quả. Vui lòng chạy thí nghiệm lượng tử trước!")
    st.stop()
...
with tab1:
    ...
    # 2. QAOA simulator — optimal weights
    ...
    # 3. Hardware — tính từ best_bitstring thực
    if hw_data:
        hw_best_bitstring = hw_data.get("best_bitstring", "")
        if hw_best_bitstring and len(hw_best_bitstring) == len(selected_tickers):
            x_hw = np.array([int(b) for b in hw_best_bitstring])
            w_hw, ret_hw, vol_hw = _optimize_weights_for_selection(
                x_hw, mean_returns.values, cov_matrix.values, risk_aversion
            )
            ...
            selected_hw = qaoa_live.get(2, {}).get("selected", [])
        ...
        summary.append({
            ...
            "Ghi chú": (
                f"Chọn: {', '.join(selected_hw)} | "
                f"Mitigated Energy: {hw_data.get('zne_mitigated_energy', 0):.4f}"
            ),
        })
...
with tab2:
    ...
    qaoa_json = comp_data.get("qaoa_by_p", {})
    if qaoa_json:
        fig_conv = go.Figure()
        colors = {"1": "#3b82f6", "2": "#8b5cf6", "3": "#ec4899"}
        for p, data in qaoa_json.items():
            convergence = data.get("convergence", [])
            if convergence:
                fig_conv.add_trace(go.Scatter(
                    y=convergence, mode="lines+markers",
                    name=f"QAOA p={p}",
                    line=dict(color=colors.get(p, "gray")),
                ))
        fig_conv.add_hline(
            y=comp_data.get("brute_force", {}).get("energy", -2.1987),
        ...
        st.info("Không có dữ liệu convergence trong comparison_results.json.")
...
with tab3:
    ...
    with c1:
        ...
        if counts:
            ...
            fig_counts = px.bar(
                df_counts, x="Bitstring", y="Probability (%)",
    ...
    with c2:
        ...
        if noise_levels:
            ...
            fig_zne = px.scatter(df_zne, x="Noise Scale", y="Energy", title="Zero Noise Extrapolation (ZNE)")
            ...
            fig_zne.add_trace(go.Scatter(
                ...
                mode="lines", line=dict(dash="dot", color="blue"), name="Extrapolation",
            ))
            fig_zne.add_trace(go.Scatter(
                x=[0], y=[mitigated], mode="markers+text",
                marker=dict(size=14, color="green", symbol="star"),
                text=["Mitigated (Scale 0)"], textposition="bottom right", name="Mitigated",
            ))
            fig_zne.add_trace(go.Scatter(
                x=[0], y=[ideal], mode="markers+text",
                marker=dict(size=14, color="red", symbol="x"),
                text=["Ideal (Simulator)"], textposition="top right", name="Ideal",
            ))
            fig_zne.update_layout(template="plotly_white")
            st.plotly_chart(fig_zne, use_container_width=True)
            st.success(
                f"ZNE giúp cải thiện Energy từ {df_zne.iloc[0]['Energy']:.4f} (Raw HW) "
                f"xuống {mitigated:.4f} (Gần hơn với lý tưởng {ideal:.4f})."
            )